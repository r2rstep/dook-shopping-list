...
class FridgeContentChanged(Event):
    fridge_id: int
    changed_products: List[ProductInFridge]
...
class FridgeLogic:
    ...
    def update_contents(self, products: List[ProductInFridge]):
        ...
        for changed_product in products:
            ...
            if changed_product.name in current_products:
                changed_product.allocated_quantity = current_products[changed_product.name].allocated_quantity
            current_products.update({changed_product.name: changed_product})
    ...
    def _add_changed_product_to_event(self, changed_product, current_products, event):
        event.changed_products.append(ProductInFridge(**changed_product.dict()))
        if changed_product.name in current_products:
